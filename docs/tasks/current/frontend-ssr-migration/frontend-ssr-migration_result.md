...
### Статус: Реализовано, не проверено на целевом устройстве
...
- **Реальное воспроизведение не проверено.** Поднять кластер не удалось: silo падает с `AddressInUseException` — порт держит кластер, запущенный ~8 часов назад в другой сессии, трогать его не стал. Не проверены на живых данных: смена треков по `ended`, срабатывание watchdog, resume-проба, доставка диагностики, ротация картинок.
