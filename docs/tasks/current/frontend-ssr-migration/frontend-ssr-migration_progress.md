...
### 04:00 Деплой и проверка в проде
Первый деплой упал не из-за переезда: `Microsoft.CodeAnalysis.*` уехал на 5.9.0, а в закреплённом образе SDK `10.0.201` компилятор 5.3.0, поэтому генератор из `Tools/Generators` отбрасывался с warning CS9057, а вместе с ним исчезал `GeneratedStatesRegistration` (CS0103 в `Extensions.csproj`). Замерено по образам: `10.0.201` -> Roslyn 5.3.0, `10.0.400` -> Roslyn 5.9.0. По решению пользователя поднят `DOTNET_SDK_TAG` до `10.0.400`, версии пакетов не трогали.

Локально проблема не воспроизводится: системный `csc` из Arch-сборки dotnet рапортует версию `5.0.0-dev` с assembly version `42.42.42.42424`, из-за чего проверка совместимости анализатора не срабатывает вовсе. Ловушка на будущее: локальная сборка эту поломку не покажет.

Проверка на задеплоенном проде: воспроизведение работает end-to-end (`paused=false`, `currentTime=19.5s`, `readyState=4`), очередь держит 3 трека, `OnTrackStarted` доставляет название трека в UI, картинки крутятся. Регрессия с `blazor.web.js` на новом SDK не повторилась.

Ранее наблюдавшийся локально `Playlist empty` оказался следствием падавшего meta, а не багом `EnsureQueue`.

Первое открытие прода после деплоя дало 404 на `blazor.webassembly.js` — это отдавал закэшированный браузером старый `index.html`, а не сервер: живой ответ содержит `blazor.web.js` и `radio-player.js`.
