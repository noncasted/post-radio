...
- Страница Console `/images` превращена в менеджер картинок: count в заголовке, actions, пустое состояние, compact grid preview 7x3 и pagination по 21 элементу без скролла страницы.
- Добавлено отдельное upload-окно в стилистике Console cards с drag-and-drop зоной, выбором нескольких файлов и выбором папки через browser file picker; фон upload modal затемнен так же, как preview.
- Добавлен полноэкранный просмотр картинки на затемненном фоне с закрытием по фону/крестику, download и delete.
- Устранен срыв Blazor Server circuit при выборе больших партий файлов/папок: загрузка переведена на browser HTTP upload по одному файлу, добавлен server endpoint, увеличен SignalR receive limit как дополнительная защита.
...
| `backend/Orchestration/Extensions/ProjectsSetupExtensions.cs` | Увеличен SignalR receive limit для Blazor Server uploads |
| `backend/Orchestration/ConsoleGateway/ConsoleMediaEndpoints.cs` | Новые endpoints для preview/download/archive/upload |
| `backend/Orchestration/ConsoleGateway/Program.cs` | Подключены console media endpoints |
| `backend/Orchestration/ConsoleGateway/App.razor` | Добавлены JS helpers для download/clipboard/per-file HTTP upload |
...
- `dotnet build backend/post-radio.slnx` после fix large batch upload — успешно, 0 warnings, 0 errors.
