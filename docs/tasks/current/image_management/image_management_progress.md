...
### 00:17 Large batch upload guard
Исправлена причина падения console при выборе >100 картинок: увеличен SignalR hub receive limit для Blazor Server metadata payload до 10 MB и добавлен top-level catch в `UploadFiles`, чтобы ошибки старта/пакета не оставляли UI в spinner-состоянии.

### 00:18 Folder upload hardening and denser grid
Folder/file upload переведен с Blazor `InputFile` streaming на обычный browser `input` + per-file HTTP `XMLHttpRequest` в `/console/media/images/upload`, чтобы большие папки не шли через SignalR circuit. Сохранен общий progress bar, добавлен server endpoint с size/form limits. Upload modal получил такое же затемнение, как preview. Grid расширен до 7 колонок x 3 строки, page size увеличен до 21.
