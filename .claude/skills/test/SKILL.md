...
2. Create the csproj with Microsoft Testing Platform configured — see `docs/TESTING.md` for the required properties if/when that doc is re-added. Minimal requirement for readable logs:
```xml
<PropertyGroup>
...
See `docs/CLAUDE_MISTAKES.md` lesson 1. Orphan `new Lifetime()` in tests leaks background loops on test failure. Always use `handle.Lifetime` or `handle.Lifetime.Child()`.
