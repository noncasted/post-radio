...
TRIGGERS: /start-cluster, start cluster, запусти кластер, start backend, run cluster
...
In prod Aspire is not used — see `docs/DEPLOY.md` (Coolify + `backend/Tools/deploy/docker-compose.yaml`).
...
- `appsettings.json` / env vars configured per `docs/DEPLOY.md`.
