...
4. From the numstat output, compute three categories:
   ...
   - **.md**: files matching `*.md` that are NOT inside `.codex/` or `.claude/`
   - **.codex**: files with paths starting with `.codex/` (including .md files inside it)
   - **.claude**: legacy files with paths starting with `.claude/` (if present)
