---
name: check
description: Run post-radio backend code review checks for modified or specified C# and Razor files. Use when the user invokes /check, asks to validate changed code with review checklists, or wants parallel/static review of backend, Blazor, Orleans, lifetime, transaction, logging, style, or docs risks.
---
...
Use Codex subagents only when the user explicitly asks for parallel agents or delegated review. Otherwise, perform the selected checks locally using the same checklists from `.codex/agents/*`.
...
> Из: docs-checker
`.codex/docs/COMMON_ORLEANS.md` — Key Files: нет ссылки на UserGrain
