---
name: validate
description: Validate modified post-radio C# and Razor files against local backend rules without broad implementation work. Use when the user invokes /validate or asks for quick in-place validation of changed files.
---
...
Always read:
- `.codex/docs/CODE_STYLE_FULL.md`
- `.codex/docs/CLAUDE_MISTAKES.md`

Read conditionally:
- `.codex/docs/COMMON_LIFETIMES.md` — if any file uses `Advise`, `View`, `ListenQueue`, `Lifetime`, `Updated.Advise`.
- `.codex/docs/COMMON_ORLEANS.md` — if any file contains `Grain`, `State<`, `[State]`, `[Transaction]`, `StateCollection`, `IOrleans`.
- `.codex/docs/BLAZOR.md` — if changeset contains `.razor`.
