---
name: task
description: Turn a raw post-radio backend request into a structured implementation brief without implementing it. Use when the user invokes /task or asks to create a task plan, task brief, or implementation brief from a description.
---
...
|-----------------|---------|
| Grain, State\<T\>, `[Transaction]`, IOrleans, IMessaging, StateCollection, jsonb | `.codex/docs/COMMON_ORLEANS.md` |
| `Advise`, `View`, `Lifetime`, `ListenQueue`, `Updated`, DeployLifetime | `.codex/docs/COMMON_LIFETIMES.md` |
| Blazor, razor, `@inject`, `[Inject]`, UiComponent, ToastService | `.codex/docs/BLAZOR.md` |
| DeployId, DeployLifetime, DeployIdPipe, Coolify, docker-compose, PgBouncer, Aspire AppHost | `.codex/docs/DEPLOY.md` |
| deploy failure, prod issue, compose logs | `.codex/docs/DEPLOY_TROUBLESHOOTING.md` |
| member order, naming, braces, nullable, Task vs UniTask | `.codex/docs/CODE_STYLE_FULL.md` |
| telemetry, metrics, FileLoggerProvider, .telemetry | `.codex/docs/TELEMETRY.md` |
| past mistake, known anti-pattern | `.codex/docs/CLAUDE_MISTAKES.md` |
| terminology, primary term | `.codex/docs/VOCABULARY.md` |
| error lookup, fix recipe | `.codex/docs/ERRORS.md` |
...
- `.codex/docs/COMMON_ORLEANS.md` — [конкретная причина: добавляем новый grain]
- `.codex/docs/DEPLOY.md` — [конкретная причина: меняем compose]
