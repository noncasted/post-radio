...
public class MessagePipeObserver : IMessagePipeObserver
{
    ...
    public Task Send(object message)
    {
        ...
        try
        {
            ...
            _logger.LogDebug("[Messaging] [Pipe] Successfully processed one-way message {MessageType}", message.GetType().Name);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[Messaging] [Pipe] Failed to process one-way message {MessageType}", message.GetType().Name);
            throw;
        }
    ...
    {
        _logger.LogDebug("[Messaging] [Pipe] Processing request-response message {MessageType} expecting {ResponseType}",
            message.GetType().Name, typeof(TResponse).Name);

        if (_responseHandler == null)
        {
            _logger.LogError("[Messaging] [Pipe] No response handler bound to process message {MessageType}", message.GetType().Name);
        ...
        try
        {
            ...
            {
                _logger.LogError("[Messaging] [Pipe] Response type mismatch for message {MessageType}. Expected {ExpectedType}, but got {ActualType}",
            ...
            _logger.LogDebug("[Messaging] [Pipe] Successfully processed request-response message {MessageType} with response {ResponseType}",
                message.GetType().Name, typeof(TResponse).Name);
        ...
        {
            _logger.LogError(ex, "[Messaging] [Pipe] Failed to process request-response message {MessageType}", message.GetType().Name);
            throw;
        }
    }
}