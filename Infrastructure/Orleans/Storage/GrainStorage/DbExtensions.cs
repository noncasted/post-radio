...
/// <summary>
///     Contains some convenience methods to use in conjunction with
///     <see cref="IRelationalStorage">IRelationalStorage</see> and
///     <see cref="RelationalStorage">GenericRelationalStorage</see>.
/// </summary>
public static class DbExtensions
{
    ...
    {
        if (EqualityComparer<T>.Default.Equals(parameters, default) == false)
        {
    ...
    public static TResult ReflectionSelector<TResult>(this IDataRecord record)
    {
        ...
        for (var i = 0; i < properties.Length; ++i)
        {
            ...
            if (Equals(rp, DBNull.Value) == false)
    ...
    extension(IDataRecord record)
    {
        ...
        /// <returns>Either the given value or the default for the requested type.</returns>
        /// <remarks>This function throws if the given <see paramref="fieldName" /> does not exist.</remarks>
        public TValue GetValueOrDefault<TValue>(string fieldName, TValue @default = default)
        {
            try
            {
                var ordinal = record.GetOrdinal(fieldName);
        ...
        /// <exception cref="DataException" />
        /// <remarks>
        ///     An explicit function like this is needed in cases where to connector infers a type that is undesirable.
        ///     An example here is Npgsql.NodaTime, which makes Npgsql return Noda type and consequently Orleans is not able to
        ///     use it since it expects .NET <see cref="System.DateTime" />. This function throws if the given
        ///     <see paramref="fieldName" /> does not exist.
        /// </remarks>
